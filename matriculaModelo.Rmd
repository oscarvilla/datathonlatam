...
```{r, correlaciones_entrePIB, echo=FALSE, cache=TRUE}
matr_tecpro_pred <- matr_tecpro[, -38]
numericData <- matr_tecpro_pred[sapply(matr_tecpro_pred, is.numeric)]
descrCor <- cor(numericData)

library(corrplot)
corrplot(descrCor, order = "original", tl.cex = 0.6, tl.col = "black", type = "lower", diag = FALSE)
```

Debemos tener cuidado con las variables independientes que tienen altas correlaciones entre ellas


```{r, eliminar_altas_corr_PIB, echo=TRUE, cache=TRUE}
library(caret)
...
```{r, model_sin_colinearidad, echo=TRUE, cache=TRUE}
...
p <- data.frame(real = matr_tecpro$matr_tecpro, 
                ...
                n = seq(1, 165, 1))
library(ggplot2)
ggplot(data = p, aes(x = n, y = real), colour = "black") + geom_point() + geom_point(aes(x = n, y = m1), colour = "blue") + geom_point(aes(x = n, y = m2), colour = "red") + ggtitle("Valores reales vs predicciones \n negro = real, azul = modelo1, rojo = modelo2")
```
